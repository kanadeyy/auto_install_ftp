...
def todoaddon():
    ...
    while True:
        try:
            addon.sendServerprogramlist(IPaddress_global)
            time.sleep(1)
        except:
            pass

def uploaduserselectedprogram():
    global IPaddress_global
    while True:
        useritem, addr = addon.receiveduserselectedprogram(IPaddress_global)
        if len(useritem) != 0:
            print(useritem)
            print(addr)
            ftp_server.copyfolder(useritem)
            Sendlink.sendlinktoaddon(useritem, addr)
            del useritem[:]
        else:
            pass
...
if __name__ == "__main__":
    ...
    srv2 = threading.Thread(target=todoaddon)
    srv2.daemon = True
    srv2.start()
    ...
    srv3 = threading.Thread(target=uploaduserselectedprogram)
    srv3.daemon = True
    srv3.start()


    # srv3 = threading.Thread(target=WindowClass.uploaduserselectedprogram)
    # srv2.daemon = True
    # srv3.start()
